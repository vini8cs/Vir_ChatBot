    ...
    def _chunking_pdfs(self):
        ...
        def process_row(row):
            ...
            if isinstance(summarized, dict):
                ...
                row["summary"] = summarized.get("summary", None)
            else:
                row["summary"] = None
            return row
        ...
        for pdf_path in self.pdf_paths:
            ...
            texts_df["summarized_content"] = texts_df["contents"].apply(
                self._generate_text_summaries
            )
            tables_df["summarized_content"] = tables_df["contents"].apply(
                self._generate_text_summaries
            )
            images_df["summarized_content"] = images_df["contents"].apply(
                self._genenate_image_summaries
            )
        ...
        self.merged_df = pd.concat(
            [self.texts_all_df, self.tables_all_df, self.images_all_df], ignore_index=True
        )